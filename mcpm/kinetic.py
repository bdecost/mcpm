...
def site_event(site, neighbors, nearest, kT, weights, sites, propensity):
...
def all_propensity(sites, neighbors, nearest, kT, weights):
...
def iterate(sites, weights, options):
  ...
  while time < length:
    ...
    while inner_time < dump_frequency:
      ...
      site_event(site, neighbors, nearest, kT, weights, sites.ravel(), propensity)
